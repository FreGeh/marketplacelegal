...
export default function Home() {
  ...
  const handleSubmit = (e: FormEvent) => {
    ...
    const thinkingMsg: Message = { id: Date.now() + 1, role: "bot", content: "Denkt nach…" };
    ...
    setTimeout(() => {
      setMessages((m) =>
        m.map((msg) =>
          msg.id === thinkingMsg.id ? { ...msg, content: mockResponses[responseIndex] } : msg
        )
      );
    ...
    <div className="flex min-h-screen flex-col bg-gray-50">
      ...
      <section className="container mx-auto flex flex-col items-center justify-center gap-8 px-5 py-24 text-center">
        ...
        <p className="text-muted-foreground max-w-2xl text-2xl">
          Sofortige Problemlösungen per KI‑Apps – bei Spezialfragen unkompliziert
          mit Top‑Fachanwälten vernetzt.
        </p>
      </section>

      {/* KI‑App‑Katalog */}
      <section className="container mx-auto mb-40 px-6"> {/*  mb‑40 lässt Platz für Chat‑Fenster */}
          ...
          {Array(2)
            ...
            .map((_, i) => (
              ...
              >
                ...
                <h3 className="text-xl font-medium text-gray-500">Bald verfügbar</h3>
              </div>
            ))}
        </div>
      </section>
      ...
      <div className="fixed bottom-0 left-0 right-0 z-40 h-96 border-t bg-white shadow-lg">
        <div className="mx-auto flex h-full max-w-3xl flex-col">
          ...
          <div ref={scrollRef} className="flex-1 space-y-3 overflow-y-auto bg-gray-50 p-4">
            {messages.length === 0 && (
              <p className="italic text-gray-400">Stelle LegalLexi deine Frage…</p>
            )}
            {messages.map((msg) => (
              <div
                key={msg.id}
          ...
          >
            ...
            />
            <button type="button" className="rounded-full bg-gray-200 p-2 hover:bg-gray-300">
